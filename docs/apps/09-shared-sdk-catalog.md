...
| Control faces | `app-theme.css` (class-only) | R + D | The shared control-box primitives, all on the `--control-height-*` scale (sm 24 · md 32 · lg 40) so any two controls line up **pixel-exact** beside each other: `.bs-btn` (buttons; `--secondary/--neutral/--ghost/--danger/--icon`, `data-bs-primary` gloss), `.bs-select` (the select trigger, above), and **`.bs-input`** (text inputs / `<textarea>` / field-trigger buttons like a date picker; `--sm/--lg/--multiline`). A control's height comes from the scale — **never** from hand-rolled padding; an app supplies only layout deltas (flex / width). Native `<input>`/`<textarea>`/`<select>` without a face is a design-system violation, enforced by `tools/check-control-faces.mjs` (shrinking baseline — down to 2 documented exceptions as of DS-input-2: the Agent + Mailbox compose `<textarea>`s). **Composite fields** (a searchbar-style icon + input sharing one box): put `.bs-input` on the WRAPPER (`<label>`/`<div>`) — it owns the box and the `:focus-within` ring — and mark the inner control `.bs-input__control` (a bare, flexible slot that inherits the field's type). The ratchet accepts `bs-input__control` as a face token, so this is the sanctioned way to nest an icon in a `.bs-input`, not a per-app opt-out. Alongside them, **`.bs-drag-grip`** is the row drag-handle face: the hover-revealed reorder / cross-app drag grip (the six-dot `IconName.DragHandle`) parked in a list/table row's left gutter — it owns the box, grab cursor, hidden→revealed transition and the absolute pin, so it reads + behaves identically everywhere; the host supplies only the gutter offset (`--bs-drag-grip-inset`) + its one row-hover reveal rule, and **must reserve a leading gutter** so the grip parks clear of the first cell (never overlaying the content). |
| Layout view | `/layout-view` (+ `/layout-view.css`) | R | The `brainstorm/Layout/v1` **render pipeline** (8.3). `<LayoutView layout entity propertyDef values>` paints a resolved `LayoutDef`: property cells through the shared cell registry (a cell's `display` overrides the property's default view), `group` cells with their own nested mode, `text` / `divider` inline, and two host seams — `renderBlock` (the BlockEmbed bridge is app-side) and `renderChrome` (see below). Modes are CSS: stacked flex column · grid from `{col,row,colSpan,rowSpan}` (collapses to stacked under 640px) · freeform absolute canvas that sizes to its content. **DOM order is reading order** — visual position is CSS-only, so `Tab` + screen readers follow doc 27's a11y contract by construction. **Per-cell subscriptions:** `LayoutValueSource` is `get` + per-key `subscribe`; each cell reads its own key through `useSyncExternalStore`, so a write repaints one cell, not the layout. `createLayoutValueSource` is the in-memory implementation for non-Yjs hosts. **Two host seams beyond block/chrome:** `renderCell(cell, body)` wraps a cell in host chrome (a field label, a required marker, a validation message) — the layout owns *which* cells exist and *where*, a host owns what surrounds one; and `renderMissingCell(cell, def)` renders a property cell the registry can't draw, which a host that must stay editable (a form) supplies rather than accept the read-only placeholder. |
| Layout chrome | `/layout-chrome` (+ `/layout-chrome.css`) | R | The six **shell-rendered chrome cells** (8.4): `actionBar` · `breadcrumb` · `meta` · `windowControls` · `entityHeader` · `tabs`. Wire with `chromeSeam(host)` → `<LayoutView seams.renderChrome>`. **The set is shell-curated and closed (OQ-90 (a))** — an app that wants its own structural element uses a `block` cell, which is honest about not being shell chrome. The SDK owns the kinds and their rendering; the **host owns the data** (its actions / crumbs / tabs) — and `More` opens the shared object menu via the host, so the SDK never owns menu policy. Each kind reads its options through a typed reader that falls back rather than faults. |
| Panel toggle | `/panel-toggle` | R + D | The sidebar / inspector collapse-expand button + icon. `<PanelToggleButton>` (React) and `createPanelToggleButton` (DOM) are exact twins — same `.bs-panel-toggle` chrome, same `aria-pressed` + state-tracking show/hide label, same `disabled` + `hint` contract. No app builds its own (`panelToggleIcon` is the helper's private glyph, not an app-facing export). **Availability rule: a `PanelSide.Right` toggle must declare whether it can act** — pass `disabled` (plus a `hint` naming what to select first, since a disabled control fires no pointer events and can't open the tooltip chip), *or* make the panel always live (its own empty state, or a CSS collapse) and baseline the file. A toggle that stays enabled while its panel is selection-gated is the recurring "dead button" bug. Enforced by `tools/check-panel-toggles.mjs` (`bun run lint`) — see [Panel-toggle availability](#panel-toggle-availability). |
...
| Markdown | `/markdown` (+ `/markdown.css`) | R | The ONE markdown→view path. `parseMarkdown` (pure block parser, canonical home) + `<Markdown source onEntityLink className>` — an XSS-safe React renderer (every string is a text node, never an HTML fragment) over `.bs-markdown` prose. Subset: `#`–`####` headings, paragraphs, fenced code, bullet/ordered lists, `---`, inline `code`/**bold**/*italic*/links; `onEntityLink` routes non-web `[text](id)` targets to a click handler (the Agent opens citations). Preview's DOM builder imports the same parser. Used by: agent (reply rendering). |
...
| Entity title | `/entity-title` | P | **The ONE answer to "what do we call this object?"** — `resolveEntityTitle(properties)` walks `title` → `name` → `displayName` → `label` → the **leaf** of `path` (`src/lib/main.ts` → `main.ts`), skipping blank / whitespace-only / non-string values, and returns `null` when the object carries no name at all. The FALLBACK stays with the caller, because it genuinely differs per surface (a canvas node wants `"Note (untitled)"`, a picker drops the row, the search collector wants `""`) — `entityTitleOr(properties, fallback)` is the one-liner for that, `hasEntityTitle` for sorts/pickers that must tell named from nameless. Type-agnostic on purpose, no per-type allowlist: a Note has `title`, a Person/File/Folder `name`, a `brainstorm/Profile/v1` ONLY `displayName`, a `brainstorm/CodeFile/v1` ONLY `path`. **Every surface that paints an entity consumes this** — Files tiles, Graph nodes, Database rows/cards, Calendar chips, Journal backlinks, the editor's `@`-mention + entity-title index, the property-ui Link cell, the agent's chips + grounding block, the BP block bundles, and the shell's search / bin / dashboard-pin `deriveEntityTitle`. A private chain is what let the fleet drift (Files read `name` only, so every Note and every CodeFile rendered `(untitled)` in the vault browser, while Graph preferred `name` over `title`); enforced by `tools/check-entity-title.mjs` in `bun run lint` — zero baseline, it fails on any file reading two title-shaped keys off a property bag. |
...
| System entities | `/system-entities` | P | `SystemEntityType` const + `SYSTEM_ENTITY_TYPES` set + `isSystemEntityType(type)` — the canonical "plumbing, not user content" classification (BrowsingSession, ListView, Trigger, Workflow, WorkflowRun, …). Presentation-only: type-enumerating surfaces (Graph SHOW filter, sidebars) group/dim these; never changes query semantics (F-212). Also `ChildEntityType` / `CHILD_ENTITY_TYPES` / `isChildEntityType(type)` — parent-scoped child content (Message, Comment) that default top-level browse listings may exclude (F-318; deliberate creations, so distinct from SYSTEM), and `typeDisplayName(typeId)` — singular human caption for a type id (`brainstorm/Task/v1` → "Task"; `friendlyTypeName` pluralizes on top of it; F-320 untitled-node captions). |
...
A right-hand panel (inspector / properties / members) usually needs something selected. Its header toggle does not automatically know that — so the recurring bug is an **enabled button that does nothing**: the panel can't render, the click changes no pixels, and the control reads as broken. It has been reported against more than one app and fixed one app at a time.

There are exactly three correct answers, and the call site must pick one:

1. **Disable it, and say why.** `disabled={!selection}` plus a `hint` — the wording that names what to select ("Open a note to see its properties"). The hint is not optional polish: a disabled control fires no pointer events, so the animated `.bs-tooltip` chip can never open and the native `title` is the *only* thing that explains the state. Books · Bookmarks · Chat · Code-editor · Contacts · Notes · Tasks.
2. **Make the panel always live.** Render it with its own empty state (Files' `inspector__empty` + a usable tab strip), or keep it mounted and collapse it with CSS (Preview's `preview--inspector-collapsed`, Journal's `aria-hidden`/`inert` aside). Then the toggle always changes what's on screen and disabling it would *remove* a working panel. These files are listed in `tools/panel-toggle-baseline.json` with the reason.
3. **Seed the selection on open.** Database picks the first visible row when you open the inspector with nothing selected — and disables the toggle only in the one case where there is nothing to seed (a view with no rows).

Conditionally *rendering* the toggle is a fourth answer and it is rejected: the control vanishes, the header jitters, and a user who has never selected anything never learns the panel exists. Bookmarks did this and was migrated to (1).

`tools/check-panel-toggles.mjs` (in `bun run lint` / `lint:apps`) enforces two mechanical rules: every `PanelSide.Right` toggle passes `disabled` or is baselined, and no app imports `panelToggleIcon` (the tell of a hand-rolled toggle — that is how Database shipped header toggles with hardcoded English labels that never tracked open/closed). Left-side toggles are exempt: a nav sidebar has content regardless of the selection.

## What is deliberately not shared

Over-extraction is also a smell. These stay per-app on purpose — do **not** try to unify them:

- **Properties-panel *adapters* — and the six apps that correctly stay on the primitive.** Two shared surfaces exist, one layered on the other: `@brainstorm-os/sdk/properties-panel` → **`<PropertiesPanel>`** (chrome + rows the host builds) and `@brainstorm-os/sdk/property-ui` → **`<EntityPropertiesPanel>`** (a stored `ValuesMap` → editable rows + add-menu + remove + a `canMutate` gate, wrapping the primitive). **The test for which surface an app belongs on is not "which panel does it import" or "how many rows does it show" — it is "does the app own a stored `ValuesMap`."** `<EntityPropertiesPanel>`'s write verb is `onWriteValues(next: ValuesMap)`, a *whole-bag* write. An app whose properties are a **projection of typed fields** has no bag to write it to: it would have to reverse-map the returned bag key-by-key back into typed fields — strictly worse than the per-key `onChange` the primitive already gives it. Journal, Preview, Notes and Tasks own a bag and use the complete panel (Notes / Tasks migrated 2026-07-28, implementation-plan Props-3 / Props-4 — Tasks rides its typed bridge rows in via the panel's `extraRows` host-row slot, merged ahead of the bag rows in the same grid, and excludes `assigneeId` from the picker via `pickerExcludeKeys`; Notes threads its localized picker labels via `pickerLabels`). The six below are **deliberate non-migrators — audited 2026-07-25, do not re-file**:
