...
> **Open:** does the shell expose a single shared webhook endpoint with per-workflow path routing (`https://<relay>/wh/<workflowId>/<secret>`), or one endpoint per workflow? Single-endpoint is simpler to operate; per-workflow is what users expect from n8n. Tentative leaning: single shared endpoint, path-routed, with per-workflow rotating secrets. Tracked as OQ-163.
...
- **Webhook triggers**: the network broker forwards a matched inbound to the scheduler; per-workflow secrets verified there.
