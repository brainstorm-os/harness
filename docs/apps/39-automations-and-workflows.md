...
- **File-watch triggers** (11b.10): the files host service (Stage 9) emits change events on granted handles; the automations host taps that stream (via the composed `emitWatch`) and runs the bound workflow, same enqueue path — the designated automation-host device only. Because file **handles are session-only** (re-granted each vault open, the pick is the audit trail), a FileWatch trigger persists its file pick as an opaque **file-watch grant** (`registry.db` `file_watch_grants`: `watchId → path`, shell-internal, **device-local — never syncs**, revocable in Settings → Privacy). The trigger config stores only the `watchId`; on each vault open the shell re-resolves it and re-mints a live handle to watch (surviving a restart). The path never reaches the app. Cross-device the `Trigger/v1` syncs but the grant does not, so an unresolvable `watchId` fails closed (no watch) — correct, since a path is device-specific.
