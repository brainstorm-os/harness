...
- **OQ-WV-5** — Does the agent's `web.browse` get a *narrower* sub-capability (e.g. `web.browse:read-only`, no form submission) so an autonomous loop can't POST to arbitrary endpoints? **RESOLVED (Browser-8, 2026-07-24) → yes.** `web.browse:read-only` is a real, separate scope — `capabilityImplies` treats an unscoped cap and a scoped one as disjoint, so granting `web.browse` never silently confers it and holding only it can never widen. The mode a view runs in is derived from the capabilities the **broker verified on the opening call** (never a caller-passed flag), fixed once at open, and preserved across a suspension remount. Enforcement is structural, not advisory: a read-only view runs in its **own throwaway partition** and its session refuses every non-GET/HEAD request, so "the loop was talked into submitting the form" is not a reachable state. Interactive submission stays the user's, in a normal `web.browse` tab.
