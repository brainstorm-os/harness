...
**Three feeders, one core, one object** (`Bookmark/v1`). Only the HTML *source* differs:
...
3. **External-browser feeder — the web clipper** (Clip-1, post-v1; see *Web clipper (later phase)*). The user's own logged-in browser hands the shell the URL or its rendered+authed DOM; the same core, the same `Bookmark/v1`. Reaches authed/paywalled pages the static feeder cannot. *Highly used* — reinstated per user directive 2026-05-19.

> **Decision:** the extraction + sanitization + HTML→Lexical logic is **not duplicated** and the **write target is the same** for all three feeders: a `Bookmark/v1` (OG → properties, page → Lexical body). Same extractor, same object, three callers. The Bookmarks v1 surface does **not wait** for the in-app browser or the clipper — it ships on the static feeder now; the live-DOM (Net-3) and external-clipper (Clip-1) feeders are additive later channels onto the identical core and object.
...
- **Web-clipper REINSTATED as a later phase** (user directive 2026-05-19 — it is highly used). Doc 54 had the in-app browser *subsume* the external-browser web-clipper; that stance is **re-opened**: the clipper is a genuinely distinct, high-traffic channel (the user clips from *their own* logged-in browser, capturing authed/paywalled pages the static feeder structurally cannot). It is **not a new object or type** — it is a third *feeder* onto the same shared extraction core, producing the same `Bookmark/v1`. Scoped post-v1 as **Clip-1** (see *Web clipper (later phase)*). The in-app browser ([54](54-web-browser.md)) and the external clipper are complementary, not substitutes; doc 54's "subsumes web-clipper" line is superseded — reconciled at Browser-1 / Clip-1.
...
## Web clipper (separate development — removed from the release 2026-05-30)

> **Decision (user directive 2026-05-30):** the web clipper is **removed from the v1/beta release plan** and is a **separate development effort** (out of this repo's tracked rungs). It was already scoped as a post-v1 feeder (Clip-1); it is now fully out of release scope. The design below is preserved for when that effort starts. Removing it costs the release nothing — it is purely an *additive* third feeder onto the unchanged Net-2 extraction core, producing the same `Bookmark/v1`; nothing in v1 depends on it. **OQ-RX-7 no longer gates any release work.**
...
| **Web clipper** — external-browser extension → shell clip receiver → same core → **same `Bookmark/v1`** — **Clip-1** | — | ✓ (highly used; OQ-RX-7; reinstated 2026-05-19) |
...
- **One extraction core, three feeders, one object:** static `network.readable` (Bookmarks, ships first — Net-2), live-DOM `web.capture` (in-app Web Browser, later — Net-3), and the **external-browser web clipper** (highly used, later — Clip-1, reinstated 2026-05-19) all produce the same `Bookmark`. No duplicated extractor, no second type; the clipper is an additive channel, not a Bookmarks-v1 dependency.
