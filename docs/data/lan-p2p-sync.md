...
> **Superseding note (2026-07-31).** The `P2P-0` spike has reported: [platform/79-p2p-sync.md](../platform/79-p2p-sync.md). It extends this doc rather than replacing it, and corrects two points. **(1) §3.2's discovery recommendation is now gated:** automatic dialing of a *discovered* address discloses the device Ed25519 pubkey in cleartext, because `hello` is the client's first message, so an mDNS advertiser must prove it belongs to your identity **before** the dial (a rotating tag in the TXT, `OQ-P2P-5`). T6 anticipated the beacon side of this, not the dial side. **(2) §3.2's "no discovery code on the critical path" claim assumed `LAN-3` wired the listener URL into the pairing payload; it did not.** `pairing-service.ts` still reads `vault.json.syncRelay.url`, and `LanHostController.onUrlChanged` has one production consumer, a `console.info`. Also settled there: `OQ-LAN-6` (coexistence) resolves to **exclusive transport, LAN preferred**, and full NAT traversal is explicitly not planned.
