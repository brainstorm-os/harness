...
- **OQ-LAN-1 — Transport shape: Option A (embedded blind relay + reuse `WebSocketRelayPort`) vs Option B (symmetric `LanRelayPort`)?** *Leaning A for MVP* (max reuse), B as evolution. Blocks LAN-2.
- **OQ-LAN-2 — Host election on a 2-peer LAN.** Who binds the listener? Candidates: (a) the mDNS advertiser hosts; (b) lower device pubkey hosts (deterministic tie-break); (c) whoever opened the shared entity first. Interacts with T8 (listener lifecycle). Blocks LAN-3.
- **OQ-LAN-3 — Is transport TLS (pairing-derived PSK) required for v1, or is the admission challenge + E2EE sufficient?** *Leaning: challenge + E2EE sufficient for v1; PSK-TLS is defense-in-depth for T3/T6 metadata, deferred.* Non-blocking for MVP.
...
- **OQ-LAN-5 — Backfill without a durable node.** The cloud path has a durable node serving `requestCatalog`/bundled backfill for offline gaps. A LAN peer is *live-only* (no durable store). Does the MVP require any backfill (edits made while a peer was off-LAN), or is live-only acceptable for the wedge? *Leaning: live-only MVP; offline gaps heal on the next cloud sync or via `SYNC-2`'s durable store when both are also cloud-connected.* Non-blocking for the demo.
...
## 6. Smallest end-to-end MVP slice

**Goal:** two devices, same LAN, already paired, one shared doc syncing live edits + presence, with **no cloud relay reachable**. Prove the wedge.

**Chosen shape:** Option A (embedded blind relay on the host + existing `WebSocketRelayPort` clients), pairing-payload address for bootstrap (defer mDNS to a follow-on rung), admission challenge as the auth gate.

### File-by-file work list

| Rung | File(s) | Change |
|---|---|---|
| **LAN-1** *(host)* | `packages/shell/src/main/sync/lan-relay-host.ts` **(new)** | Wrap `@brainstorm-os/relay-server`'s `createRelayCore` in a real socket server (Bun `serve` / Node `ws`) bound to a LAN interface + a random high port. Expose `start(): {url}` / `stop()`. **Relay-blind (named `*relay*` → inherits the CI fence): no crypto imports.** Reuses `FrameRouter` fan-out verbatim. |
| **LAN-2** *(admission gate)* | `packages/shell/src/main/sync/lan-admission.ts` **(new)** + a hook into LAN-1's server accept path | The §4.3 challenge: on connect, host sends `challenge{nonce}` (existing control op); client answers via the existing `onChallenge` → `challenge-responder.ts` (already wired at `main/index.ts`); host verifies the signature against the signed device roster (`meta.devices`) and admits or closes. Auth logic lives **outside** the relay-blind fence, injected into LAN-1 as a `verifyAdmission(account, sig, nonce)` callback. Client-side mutual check (T4) reuses the same primitive in reverse. |
| **LAN-3** *(discovery bootstrap)* | `main/pairing/pairing-handshake.ts` + `pairing-service.ts` (touch) | Populate `relayUrl` with the host's `ws://<lan-ip>:<port>` when LAN mode is chosen at pair time. No new module — the field + transport seam already exist. |
| **LAN-4** *(transport selection)* | `main/sync/active-relay.ts` + `main/index.ts` (touch) | Add `ActiveRelayKind.Lan` (label only, for the sync-status panel). Start the LAN host (LAN-1) when a LAN URL is resolved; point the local `WebSocketRelayPort` at `ws://127.0.0.1:port` on the host and `ws://<peer-ip>:port` on the guest. `makeRelayPort` already builds `WebSocketRelayPort` — this only chooses the URL + starts/stops the host alongside the session. Tie host lifecycle to a shared+open entity (T8). |
| **LAN-5** *(status UX)* | `main/sync/sync-status-store.ts` + the dashboard sync-status panel (touch) | Surface "Syncing on local network (no server)" vs "Syncing via relay" so the wedge is *visible* — this is the pitch. |
| **LAN-6** *(tests)* | `main/sync/lan-relay-host.test.ts`, `lan-admission.test.ts` **(new)**; extend `main/integration/*` | Two `LiveSyncEngine`s over a real localhost `lan-relay-host` (two `WebSocketRelayPort`s to it) reproduce a live co-edit + a presence update with **no cloud relay** — the in-process analog of the demo. Admission: a non-roster signer is rejected; a roster signer is admitted; a forged nonce sig fails. Property/round-trip tests reuse the existing envelope suites unchanged (the transport is opaque to them). |

**What is deliberately NOT in the MVP:** mDNS auto-discovery (LAN-3 uses the pairing address; mDNS is a fast-follow rung), the blob/asset plane (`0x02`), bundled backfill (`0x03`), durable-node catalog/rotation storage, dual LAN+cloud transport (OQ-LAN-6), and TLS/PSK on the transport (OQ-LAN-3). Each is a named follow-on, not a gap.

**Pre-handoff gate:** `bun run verify` (typecheck + build + in-process pipeline) + the full `bun run test` (LAN sockets touch the reconnect/queue paths the ubuntu CI exercises), plus — because LAN-1 may add a `ws`/discovery runtime dep — an electron-builder `--dir` dry-run per the dep-changes rule, and the mandatory `/security-review` + `/pentester` pass on the admission path.
