...
- **the fix as shipped (shell!18):** re-ordering does NOT work — the whole `PairingServiceSession` captures its identity provider once (`pairing-handlers.ts:158`), so `getUserIdentity()` answers with the pre-pairing self even after the reopen. The keystore is the one place holding the adopted secret by then, so `confirmSas` reads it back through a new `loadIdentitySecret()`, signs both rows with it and with the adopted `addedBy`, and zeroes it in a `finally`. Fails closed: no adopted secret ⇒ throw, because a row signed by the old identity is worse than none (a permanent unverifiable roster entry). The deliberate reopen ordering is preserved.
- **residual, filed not silenced:** three `dropped an unverifiable roster record` lines still appear on the joiner — now the *mirror image* and harmless. They fall in the window between the roster write and the reopen, where the session still holds the OLD identity and correctly refuses rows signed with the NEW one; they stop the instant the reopen lands (verified by log ordering: drops at lines 16–18, `setActiveVaultSession` at line 20, then `[lan-dial] … (discovery)` with no refusal). A roster read that logs errors for records which are in fact correct is misleading noise and wants its own tidy-up.
...
- **session:** dev-2026-08-11 (probe 940)   **kind:** bug   **app:** shell (appearance / auto mode)   **status:** 🟡 **fixed on `fix/theme-auto-authority` (shell) + `fix/f495-theme-auto-probe` (harness) — not yet merged**
...
- **what is NOT broken:** the EXPLICIT switch. Through the Settings segmented control, the dashboard header toggle, and the IPC, **50 of 57 sampled surfaces repaint symmetrically in both directions with zero one-way failures** — dashboard, 4 app renderers, 5 tab strips, both canvases (verified by pixel hash, so the imperatively-drawn Graph/Whiteboard content really re-renders), all 8 widget iframes, the `.app-header` glass and its `::after` scrim, the Settings panel. Warm-keep revival is clean too: close an app (it parks), switch, reopen — the revived renderer picks up the CURRENT theme. **Both real controls genuinely reach the API; there is no dead control.** Every one of those was a prior hypothesis, and every one is now disproved by measurement.
- **what IS broken — the AUTO path, and it is a TORN STATE not a dead switch:** with `mode=auto` and the OS preference flipping to dark (main reporting `nativeTheme.shouldUseDarkColors=true`), the **dashboard stays LIGHT** — root token `rgb(247,247,247)`, `[data-theme=default-light]` — and **never converges across a 10s poll** (sampled every second, all light), while every app window and tab strip correctly goes dark. **Reproduced on 3 of 3 consecutive runs, always on the FIRST OS flip after entering auto**; later OS flips converge within 1s. The user sees dark apps against a light dashboard.
- **consequence, not a second bug:** all 8 dashboard widget iframes also stay light. `widgets-layer.tsx` re-pushes theme CSS from a `MutationObserver` on `:root[data-theme]`; the dashboard never changed `data-theme`, so the observer never fired. **20 of the 24 stale surfaces are dashboard + widgets.**
- **why it hid for so long:** every previous pass exercised the explicit switch, which works. The signature — first flip after entering auto fails, subsequent ones work — reads as a race, and is invisible both to reading and to any test that switches explicitly.
- **ROOT CAUSE (2026-08-11): two authorities for one decision, and a diff that suppresses the correction.** Main resolves the Auto slot from `nativeTheme.shouldUseDarkColors`; the dashboard renderer resolved it *again*, from its own `matchMedia("(prefers-color-scheme: dark)")` (`theme-provider.tsx` → `effectiveTheme(mode, prefersDark)`). The probe caught the two disagreeing inside the live shell — `renderer matchMedia dark=false` printed in the same breath as `main shouldUseDarkColors=true`, and printed again while the dashboard was painted `[data-theme=default-dark]` (notes.md lines 333 / 519). App windows and tab strips never re-derive anything — they are *told* a theme name — which is exactly why only the dashboard tore. And nothing corrected it: main broadcasts `app:theme-changed` only when the theme IT computed changes (`lastBroadcastTheme` diff in `dashboard-handlers.ts`), so once the renderer sat on the other slot the corrective push looked redundant and never went out. The "first flip fails, later ones work" signature is that diff, not a race: a flip that moves main's own value gets a push, one that doesn't, doesn't.
- **the fix (no poll, no retry):** main is the ONE authority. `DashboardStore.snapshotForOs(systemPrefersDark)` resolves the slot and ships the reading it used with the snapshot, and every renderer-bound snapshot path goes through it. The renderer reads it back through a single accessor, `useOsPrefersDark(snapshot)` (`renderer/dashboard/appearance-watcher.ts`), which falls back to `matchMedia` only before the first snapshot arrives. **Class swept:** all three renderer consumers of the old reading now use it — the `:root` theme (`theme-provider.tsx`), the header appearance toggle *and its destination mode* (`dashboard.tsx` — a disagreement there picks the wrong target mode, i.e. a dead click), and the Settings → Appearance "Active" badge (`appearance-section.tsx`). Nothing else in the shell renderer reads `prefers-color-scheme`; the two app-side readers (`code-editor` `prefersDarkScheme`, `agent` proposal tray) already prefer the shell-painted appearance and use `matchMedia` only as a no-shell fallback.
- **tests:** `packages/shell/src/renderer/theme/theme-provider.auto.test.tsx` — 5 DOM cases with `matchMedia` pinned LIGHT (the measured state); 3 fail on the unfixed code with the measured values (`expected 'default-light' to be 'default-dark'`) and one of those is the widget consequence (`expected [] to include 'default-dark'` — the `MutationObserver` on `:root[data-theme]` never fires at all, which is precisely why the 8 iframes stayed light). Plus `dashboard-store.test.ts` × 2 for `snapshotForOs`. Probe 940's phase F now ASSERTS (phases A–E stay a census): the dashboard must reach main's slot, and no surface painting a real colour may sit out an OS-driven flip.
- **how it was found:** `tests/dogfood/sessions/940-probe-theme-repaint-surfaces.spec.ts` — a **census, not an assertion**: 57 surfaces sampled before and after each switch, reporting those whose value is byte-identical across the flip. Three reading kinds, because the shell paints three kinds of surface: resolved TOKEN inside each document, COMPUTED `getComputedStyle` of named elements (a document can hold correct tokens while an element paints a stale literal), and PIXEL hashes (the only reading that can see a canvas). Six phases incl. driving the OS preference from the main process via `nativeTheme.themeSource`. Ran hidden-window, 4×.
- **the lesson:** ⚠️ a poll/retry would have "fixed" this and left the split live. When two processes each resolve the same user-visible decision from their own source, the fix is to delete one of the sources, not to reconcile them more often.

### F-496 - the Settings sidebar is a black wash in light theme
- **session:** dev-2026-08-11 (probe 940)   **kind:** bug (design)   **app:** shell (Settings)   **status:** 🟡 **fixed on `fix/theme-auto-authority` — not yet merged**
- **what happened:** `.settings__sidebar` reads `rgba(0, 0, 0, 0.18)` in dark AND light, in every phase of the census — it is **theme-invariant**.
- **root cause:** `packages/shell/src/renderer/settings/settings.css:52` hardcodes `background: rgba(0, 0, 0, 0.18)` instead of a theme token, so the nav column is a black wash over white glass in light mode.
- **the fix:** `var(--color-glass-background-strong)` — the panel is already `glass--strong`, so a second layer of the same token is denser than its host in BOTH themes, which is what the original comment said it wanted.
- **it was never one line — the wash was copy-pasted into every sibling panel.** The same `rgba(0, 0, 0, 0.18)` also painted `.help__sidebar` and `.marketplace__sidebar` (same token now), and the Marketplace card / sources-row / update-row / detail-chip surfaces (`--color-surface-default`, hover `--color-surface-overlay` — the alpha tokens that exist for "raised over glass"). Swept with it: the window-switcher backdrop (`rgba(0,0,0,0.4)` → `--color-dimmer`), the sign-out hover tint (→ `color-mix(in srgb, var(--color-state-error) 8%, transparent)`, the established pattern here), and two white swatch rings invisible on a light theme (→ `--color-glass-border`). `bun run lint` (`check-css-tokens`) is green, so every name resolves rather than rendering through a fallback.
...
- **what happened:** pairing writes the SOURCE's identity secret into the target's keystore (`saveIdentitySecret` → `backend.setSecret(vaultId, "identity", …)`, `main/ipc/pairing-handlers.ts:153`) but **nothing ever updates the target's `vault.json.identityPublicKey`** — that field is written once, at vault creation (`main/vault/vault.ts:172`), and every later writer only *preserves* it. On the next launch `openVault` passes it as `expectedPublicKeyBase64` (`vault.ts:246-248`) and `VaultSession.open` throws: `Vault <id>: identity public key in keystore does not match vault.json (possible tampering or wrong keystore).` The device cannot open its vault at all.
- **what I expected:** joining either works, or refuses up front — not "Joined", then a vault that will not open at next start.
- **evidence:** `packages/shell/src/main/pairing/pairing-identity-adoption.test.ts` (new, 2 cases, both green against real `VaultSession`s): case 1 pins that the live session keeps its pre-pairing identity (the [[F-492]] cause); case 2 pins that the next `open` rejects with `/does not match vault\.json/`.
- **the premise this rests on:** the comment at `pairing-handlers.ts:153` states the v1 assumption outright — *"the target's vault.json is the same logical vault as the source's, just opened on a different device after the user copies it across, so the keys match by construction"*. That holds only if the user manually copied the vault folder to the second machine FIRST. Nothing in the product says so, nothing enforces it, and Settings → Devices → Join is reachable from any device with its own vault (Settings requires an open vault, so the target always has one).
- **triage:** _(a design decision, not a patch. Three positions with different user-visible consequences: **(a)** pairing rewrites the target's `vault.json` identity + reopens the session — adopts the shared identity, but silently repoints an existing vault whose entities were all created under the OLD identity (`createdBy`, access grants, signatures) and orphans them; **(b)** refuse to join when the target already holds a non-empty vault, explaining that pairing joins a vault rather than merging two; **(c)** real multi-identity support — out of v1 scope. Wants an owner position + an OQ before code lands, per the "designed before it is written" rule `10.3c` set for itself.)_

### F-492 - a second device never receives a single entity key: it listens on its pre-pairing inbox
- **session:** dev-2026-08-03 (10.3c two-device dogfood, `collab/012`)   **kind:** bug   **app:** shell (sync)   **status:** 🟡 fix open in review — **shell!10**; same defect as [[F-493]], other face. `10.3c`'s producers did NOT fix this and could not have: the producer seals correctly to `inbox:<shared identity>`, but the joining device never adopts that identity, so it listens on the wrong channel. Still keeps multi-device sync out of "Shipped" on the public roadmap**
...
- **triage (SUPERSEDED, kept because it was wrong in an instructive way):** _(the fix is to re-derive the inbox from the CURRENT session identity and re-subscribe when pairing changes it — either rebuild the engine on pairing completion, or make `devicePub` a getter and have `onDevicesChanged` re-subscribe.)_
- **triage (corrected 2026-08-04):** **that fix would not have worked, and the reason is [[F-493]].** Re-deriving the inbox at pairing time recomputes the *same stale channel*, because the joining device never adopts the shared identity in-process at all: `VaultSession.identity` is `readonly`, set once in the constructor; `scanPayload` only writes the source's secret to the keystore for "the next `VaultSession.open`" (`pairing-handlers.ts:153`); and `confirmSas` signs its roster records with `session.getUserIdentity()` — the target's OWN key. Nothing reopens the vault, and the join UI goes straight to "Joined" with no restart hint. Verified in `pairing-identity-adoption.test.ts`. So F-492 is not a subscription bug — it is the visible symptom of pairing's identity adoption being incomplete, and it cannot be fixed before [[F-493]] has an owner position, because every candidate fix there changes which identity the device ends up on. Until then 10.3c is not demonstrable between two real machines and the roadmap line stays in "In progress".
...
- **evidence:** `tests/dogfood/.sessions/936-journal-persist-heal-restart-verify/notes.md` — `[ydoc] journal-2026-07-29: document has unintegratable structs (a persisted update is missing)`, same for `-07-26`; `937-probe-journal-applydoc-rejection/notes.md` shows `-07-29` as `EXISTS` + `RESOLVED`.
- **resolution (developer, 2026-08-03, shell #464 / plan `3.11`):** the sweep exists — `YDocStore.scanDamaged()` walks every shard and names each doc that is not intact, and `entities.loadDoc` now fires `onDamagedDoc` so opening one posts a notice worded not to imply recovery. **Run against the real Northbound vault it found SEVEN damaged Journal days, not the two a single session happened to open** (`journal-2026-07-09 / -14 / -17 / -18 / -23 / -26 / -29`) — which is the whole argument for a sweep: the per-load flag only ever showed you what you looked at, and this log itself carried the wrong number until it ran. That figure matches the 329 audit's original count. **What is NOT fixed and cannot be:** the content. Those bytes were never written to disk, so no repair path exists in principle, not merely in this release. Remaining work is `3.12` (a damaged body still renders as an ordinary empty page inside the app, the exact confusion that made the 329 audit read F-488 as a render fault).
- **original triage:** _( the durable-signal half of #460 already detects this; nothing consumes it yet. `loadDoc` returns `pendingStructs` over IPC and it is dropped on the floor. Two things to decide: (1) surface it — a doc that lost content must not render as an ordinary empty page, which is the whole reason the "5 words" counter over a blank pane was so confusing; (2) how many other entity types are silently in this state. The worker warning is per-load, so a boot-wide sweep would enumerate the real blast radius. Any user who wrote in Journal before 0.13.0 may have days in this state.)_

### F-490 - browsing a Journal day you never wrote in logs a "local edits are not durable" error
- **session:** dev-2026-08-03 (F-488 real-shell verification)   **kind:** bug   **app:** journal   **status:** ✅ fixed (shell #463)
- **what I was trying to do:** page back through the Journal to reach a day with no entry.
- **what happened:** each day I merely *paged past* produced five rejected full-state persists and then `[react-yjs] failed to load Y.Doc for entity journal-<date>: ydoc: canonical side rejected 5 persist attempts — local edits are not durable`. Six such errors in one session from browsing alone; I typed in none of them.
- **what I expected:** looking at an empty day is not an edit and should not error.
- **resolution (developer, 2026-08-03, shell #463):** a persist rejection now carries a *kind*, supplied by the transport because it is the only layer that knows the protocol's error shapes. `Denied` is no longer retried at all (the ledger cannot change under an open doc, so every resend re-shipped the whole document at a boundary that had already refused it); `EntityMissing` is still retried, because the real user race resolves inside the backoff, but the report no longer claims lost edits; `Transient` is unchanged, so the F-488 heal is intact. The give-up report also carries the underlying cause now — its absence is why diagnosing F-488 needed a separate probe (session 937) — and the accessor stops logging persist failures as *load* failures. Two further silent defects from the #460 review went with it: an armed resend died unreported on teardown, and the revive debt-carry keyed its re-ship on `seed`, dropping the debt on precisely the branch it existed to protect.
- **original triage:** _( introduced by shell #460, which is otherwise correct. Mounting the Lexical editor writes an initial root/paragraph into the replica, which counts as an update and ships a persist; the day has no row, so `applyDoc` answers `Invalid: not found`, and the new backoff treats that as transient and retries five times before giving up. Nothing is lost — there are no user edits in a scaffold — so the message is also wrong. Two candidate fixes: distinguish permanent rejections (`Invalid`, `Denied`) from the transient not-yet-created case rather than retrying all of them (see the same misclassification for `Denied` noted in the #460 review), or don't ship a persist for a doc the user has not touched. Note the give-up message discards the underlying error, which is why diagnosing F-488 needed a separate probe (session 937) to recover it — worth carrying the cause into the report.)_

### F-489 - a collapsed sidebar is still in the tab order: Tab lands on a panel you cannot see
- **session:** dev-2026-08-02 (329 audit, probes 934/935)   **kind:** bug (a11y)   **app:** shell (fleet — ~14 apps)   **status:** ✅ fixed (shell `4c1461c1` + `a9dfac21`, incl. the Mailbox zero-track instance) — ⚠️ **but the RATCHET that is supposed to hold the class has no teeth in 9 of 10 apps.** A skeptic reproduced it: the collapsed-track join accepts *any* block in the file carrying `visibility:hidden`/`display:none`, without checking that the guarded element is the collapsed panel. Nine of the ten zero-track rules already ship such a decoy (e.g. `.x[data-nav-open="false"] .x__nav-resize { display:none }`), so removing a real guard still passes. **No live second instance exists today** (swept: width/max-width/flex-basis/max-height/translateY/opacity — all clean), so the bug is genuinely closed; the ratchet is what needs tightening, tracked with the polish work. Recorded rather than quietly accepted, because a ratchet that certifies clean while the defect is live is precisely [[F-486]].
...
- **resolution (developer, 2026-08-10, shell 4c1461c1 + follow-up):** twenty-one rules across fourteen apps and the shared `.bs-props` primitive now declare `visibility: hidden` alongside the off-canvas `transform`, with `visibility` listed in the panel's transition (a *duration*, not a `0s linear` delay — a delay survives the universal `is-booting`/`is-resizing` suppressions, which would leave a hydrating-to-closed panel focusable for 180ms on every launch). `tools/check-offcanvas-guard.mjs` keeps it.
- **follow-up (same fix, a class the first pass missed):** the ratchet keyed on `translateX(±100%)` **only**, and Mailbox does not translate anything — it collapses its folder rail by animating the grid (`.mb-app__panes--rail-closed { grid-template-columns: 0px 340px 1fr }`), and `.mb-rail`'s own `overflow-x: hidden` clipped the buttons to invisibility while leaving every one of them rendered, tabbable and announced. So the ratchet reported *zero violations* over the one file that was still broken — the F-486 failure mode exactly. Fixed three ways: (1) Mailbox's collapsed rail carries `visibility: hidden` + `aria-hidden` + `inert`; (2) the ratchet learned the collapsed-track shape — a `grid-template-columns`/`-rows` track sized to 0 must be met by a guard rule under the SAME state token in the same file (the collapsing rule styles the grid *container*, which stays visible, so a per-block rule is unsatisfiable here); (3) the baseline is now `key → reason`, and a key with no written reason fails the run, because an allowlist that need not explain itself is a mute button. The tool's header names what it still cannot see (`inset`/`width` collapses, variable-width translates, and code-only guards). Chat and Calendar also gained the React `inert`/`aria-hidden` their panels lacked — the CSS guard rides the 180ms slide and so only lands at the *end* of a close.
- **residue:** Books is the one baselined entry. Its guard is real and doubled (`.books__library--closed` carries `visibility: hidden`, and `LibraryPanel` passes `inert` + `aria-hidden`), but it hangs off a sibling modifier class rather than a descendant of the collapsing state, so no per-file selector match can see the join.

### F-488 - Journal's entry body never renders in the main pane
- **session:** dev-2026-08-02 (329 audit, batch 1)   **kind:** bug   **app:** journal   **status:** ✅ fixed (shell #460), verified in a real shell (sessions 936/937/938)
...
- **triage / resolution (developer, 2026-08-03, shell #460 + harness sessions 936/937/938):** the original triage guessed wrong — it was **not** a render/mount fault, and the content was **not** loaded. It was data loss in the CRDT persistence path. `EntryBody` starts a day's implicit `entities.create` from the editor's `onFocus` (app.tsx:1856), so a user who focuses and immediately types races it: `entities.applyDoc` answers `Invalid: <id> not found` until the row commits, and `resolver-accessor.ts` **swallowed** that rejection. A Yjs update is a diff, not a snapshot, so the dropped one is never re-carried — the canonical doc keeps a permanent hole, every later struct that depends on it parks in `pendingStructs`, and the body renders blank forever while the denormalised row snippet still reads "5 words". That is the exact symptom pair the audit captured. #460 surfaces the rejection and answers it by re-shipping the replica's full state on a 250/500/1000/2000/4000 ms backoff (Yjs apply is idempotent, so one success heals any number of drops), and teaches `ydoc-store` to report `pendingStructs` so a damaged doc stops reading as merely empty.
  **Verified in a real shell, not just unit tests:** session 938 focuses the editor on a day with no row, types immediately with real keystrokes, waits out the full backoff, then **closes the shell and reopens the same vault cold** — the text survives, and that day is absent from the give-up log, i.e. the heal fired rather than the race never happening. Session 936 (which wrote through the `__brainstormJournalDev` dev hook) FAILED the same restart; that is a probe artifact, not a product bug — the hook injects into the captured Lexical editor and so never fires the `onFocus` that starts the create, leaving a rejection no backoff can ever satisfy. Session 937 pinned the cause by asking the service directly: `entities.get` returns NULL for such a day while a day with a row applies cleanly. **Do not use the dev hook to test durability.**
  Two residues, both filed separately and neither blocking: [[F-490]] (browsing an uncreated day now logs a give-up error) and [[F-491]] (days already damaged before this fix stay blank — the fix prevents new holes, it cannot invent the missing bytes).
...
- **refutation + close (developer, 2026-08-10, Lock-5(e)):** a skeptic pass on the (a)(b)(c) commits (`6aac0b11`, `2479c78e`, `dc34535d`) refuted the headline claim. Contacts had three write paths that never reached the gated `patchPerson`/`deletePerson` — "+ Add company" (wrote `company` onto the person directly, and the control rendered live on a locked contact), the page BODY editor (persists via `entities.applyDoc`), and the duplicates MERGE (which destroys the losers). The Books test called "refuses a property write on a locked book" performed no property write — it clicked the lock button, which bypassed `patchBook` — so deleting the guard left every Books lock test green. Files' store gates and Contacts' patch/delete gates had no tests at all, and the lock-flip exemption was key-presence based (`{locked, name}` walked a rename through). Closed by: shared `isLockOnlyPatch`/`lockRefusesWrite` (only a lock-ONLY patch is exempt; a patch-less write is refused outright), Books' and Contacts' write decisions extracted to one testable module each with every writer funnelled through them (including Books' page-turn persister and PDF metadata backfill, which both wrote a locked book with no UI in sight), and lock-declaring chrome on every Contacts page affordance. Each gate was verified by DELETING it and watching a test go red.
...
- **what happened:** nothing, again — no prompt, no toast, no window. This is the third time it's been "fixed" (F-468: #221 → #349 → #389) and it has never once worked for me.
- **what I expected:** the GitHub issue page to open, or at least be asked how to open it.
- **evidence:** `tests/dogfood/sessions/930-report-github-state-probe.spec.ts` against Northbound: the `open-with:prompt` request reaches the renderer (probe listener logs it, candidates = Browser + system default) but no picker enters the DOM; the dispatch hangs to the host's 60 s Cancel, which `openExternalUrl` silences by design.
- **triage / resolution (developer, 2026-08-02, shell #440):** `<OpenWithPromptHost />` was mounted only in the NO-vault dashboard branch, never in the active-vault tree. With a vault open and 2+ `https` openers (the Browser app + system default — every real vault) the bus raises the picker, nobody renders it, timeout → Cancel → deliberate silence. Every prior fix was real but aimed at other legs; the regression spec passed because a fresh e2e vault has no Browser app installed, so it exercises the consent prompt — whose host IS mounted in both branches. Fix: mount the host in the active-vault tree; new spec leg seeds the prebuilt apps (the real-vault shape), asserts the picker VISIBLY appears, picks "Open with system default", and asserts the URL reaches `shell.openExternal`. Verified pre/post against Northbound (probe 930; vault backed up first, probe pin reverted). **Owner note:** needs a full shell restart after merge; if you ever hit "Remember" on the picker, the pin routes links into the in-vault Browser (where GitHub is logged out) — clear it in Settings → Defaults. Residue: the prompt-host pattern has three hosts mounted per-branch by hand; a shared "mount in BOTH trees" component (or a render-tree test over the host set) would end the class.

### F-482 - the Contacts detail page looks amateur: stray placeholder at the top, dead property labels, form-stack filters, no icon/cover
- **session:** owner-report-2026-08-02 (screenshot)   **kind:** design   **app:** contacts (+ sdk properties-panel)   **status:** 🟡 fixed in shell PR #441 (awaiting merge)
- **what I was trying to do:** look at a contact.
- **what happened:** "Write notes about this person…" floats at the TOP of the content pane, the property rows are bare gray labels with a blank value column, Group-by/Sort-by stack as two chunky full-width rows above the list, properties jump when clicked, and there's no icon picking/uploading or cover — which every other entity page has.
- **what I expected:** the polish bar the other detail pages meet.
- **triage / resolution (developer, 2026-08-02, shell #441):** five distinct causes, three systemic. (1) The Lexical placeholder is absolutely positioned and Contacts had no positioned ancestor — it anchored to the whole pane; `position: relative` on the body. (2) The SDK default `cellEmpty` was `""` while Notes localised it to "Empty" — every non-Notes host silently rendered blank empties; default flipped to "Empty" + Contacts passes a localised override. (3) The inline property block inherited the panel's full-height flex/scroller/padding — Database and Files each carried the identical CSS override (rule of three) — extracted as the shared `inline` prop on `<PropertiesPanel>`, both migrated, Tasks adopted. (4) Sidebar controls compacted two-up with value-only captions (full phrasing in tooltip/aria/menu label). (5) Universal icon + cover wired the Notes/Bookmarks way (avatar = icon picker w/ upload; cover band + Add/Change-cover in the ⋯ menu; `parseIcon`/`parseCover` projection). "Properties jump" was already fixed on main Jul 31 (#395) — the owner's screenshot predates it (apps only redeploy on full shell restart). #441 also closes F-480's flagged residue: Graphite merged without its theme-editor label (typecheck:apps red on main again) — fixed there.
...
- **session:** collab-012-lan-two-devices   **kind:** bug   **app:** shell (sync)   **status:** 🟡 root cause finally identified (2026-08-10) — the chain ran LAN transport (✅ shell #394) → missing DEK producer (`10.3c` ✅ 2026-08-03) → **and it still did not sync**, because the joining device never adopts the shared identity ([[F-492]]). Fix open in **shell!10**. Three successive root causes for one symptom is why this entry stayed open across four months: each fix was real and none was sufficient.
...
- **session:** 350-marcus-agent-design-review   **kind:** design   **app:** Agent   **status:** ✅ closed won't-fix (2026-08-10) — the 2026-06-28 verify-before-fix pass already reached "keep as is"; carrying it as open since then only inflated the open count. Reopen if a user reads ★ as *favorite* in a real session, which is the only evidence that would change the call.
...
- **session:** 235-mira-operating-hub   **kind:** design   **app:** Journal   **status:** ✅ closed not-product-friction (2026-08-10) — the entry says so in its own title: the dev hook cannot reach an editor that has not mounted yet. That is a property of the harness, not something a user can hit. The durable lesson (dev hooks bypass the create-on-focus path, so they can fake durability failures) is recorded where it belongs, on the durability work.
