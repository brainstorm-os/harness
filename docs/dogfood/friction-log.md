...
- **session:** collab-003 / collab-004   **kind:** gap (harness)   **app:** tests/dogfood (launch-durable-node)   **status:** open
- **what happened:** `003-durable-node` and `004-wipe-and-restore` (the SYNC-2 real-durable-node sessions) fail at setup with `Error: spawn bun ENOENT` — the node launcher spawns `bun` by bare command name, which isn't on the PATH Playwright hands the child process. Same post-split-environment class as the import breakage from collab-005: the harness assumes a toolchain on PATH that the spawn context doesn't have. So the durable-node + cold-restore collaboration paths can't be dogfooded right now (the in-process-relay sessions 001/002/005/006 are unaffected).
- **fix direction:** spawn `bun` by an absolute/resolved path (e.g. `process.execPath` when already running under bun, or a `BUN_BIN`/`which bun` lookup) in `launch-durable-node.ts`, mirroring how `launch-relay.ts` resolves its binary.
- **triage:** _(next)_ small launcher fix; restores SYNC-2 dogfood coverage.
...
- **session:** live use (2026-06-13)   **kind:** bug   **app:** Files   **status:** 🟡 fix landed (verified session 233)
...
- **session:** live use (2026-06-13)   **kind:** bug   **app:** Books / Preview   **status:** 🟡 fix landed (in-shell PDF verify pending — no PDFs in the Northbound vault)
...
- **session:** 228-deep-agent   **kind:** gap   **app:** Agent   **status:** open
...
- **triage:** _(open — `11c`; note Ollama was unreachable this run so generation itself wasn't exercised — the gap is the missing seam, not the model.)_
...
- **session:** 227-functional-sweep-b   **kind:** bug?   **app:** Browser   **status:** open (low confidence)
...
- **session:** 200c-comment-review-retry   **kind:** design   **app:** Notes (comments) / shell identity   **status:** open
...
- **session:** 187-mira-team-roster   **kind:** design   **app:** Contacts   **status:** ✅ done (ghost prevention, 2026-06-09) — merge/de-dup remains a follow-up
