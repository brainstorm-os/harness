...
- **why it hid for so long:** every previous pass exercised the explicit switch, which works. The signature — first flip after entering auto fails, subsequent ones work — is a subscription/initial-state race, invisible to reading and invisible to any test that switches explicitly.
...
- **fix:** compare the dashboard's OS-preference path against `pushChromeTheme` / `app:theme-changed`, which take a different route and work. ⚠️ **A poll/retry is NOT an acceptable fix** — it would also "fix" a genuinely stuck state while leaving the race live.

### F-496 - the Settings sidebar is a black wash in light theme
- **session:** dev-2026-08-11 (probe 940)   **kind:** bug (design)   **app:** shell (Settings)   **status:** open — one-line fix
...
- **fix:** a real theme token (verify it exists — a phantom var renders via its fallback in the one theme you eyeball and breaks the others; `check-css-tokens` in `bun run lint` catches it). Sweep the shell renderer for other theme-invariant colour literals.
