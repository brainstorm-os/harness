...
## GA — GA / pre-1.0 (release trains 0.8.0→1.0.0) (40)
...
| `POLISH-PROP-2` | Database: number/money grid cells are left-aligned while the same column's footer aggrega… | ⚪ pending |  |
| `POLISH-DSN-3` | Browser: the new-tab page is a fully blank surface (15-app-browser.png) | ⚪ pending |  |
| `POLISH-DSN-4` | Files: the toolbar's sort trigger (.toolbar__sort, ghost text + caret) sits in the same r… | ⚪ pending |  |
| `POLISH-LAY-2` | Mailbox: the message list's "No messages" empty state is top-anchored (EmptyState Compact… | ⚪ pending |  |
| `POLISH-DSN-5` | Notes: sidebar rows mix icon-bearing and icon-less notes, so titles don't share a left ed… | ⚪ pending |  |
