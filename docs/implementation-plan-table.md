...
## GA — GA / pre-1.0 (release trains 0.8.0→1.0.0) (55)
...
| `POLISH-LAY-6` | an installed app's dashboard tile lands ON TOP of an existing app. Installing Client Puls… | ⚪ pending |  |
| `POLISH-LAY-7` | the capability-grants popover truncates the scope, which is the only part that carries me… | ⚪ pending |  |
| `POLISH-LAY-8` | brainstorm://app-icon/<id> 404s for a sideloaded app whose manifest declares no icon | ⚪ pending |  |
| `POLISH-FN-1` | Agent's "New chat" does not start a new chat when the vault already has conversations. ne… | ⚪ pending |  |
| `POLISH-FN-2` | the Code editor keeps language: "typescript" after a rename changes the extension. create… | ⚪ pending |  |
