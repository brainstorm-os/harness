...
## GA — GA / pre-1.0 (release trains 0.8.0→1.0.0) (37)

### Sync, multi-device & E2E encryption *(Stage 10)*

| ID | Task | Status | Gate |
| -- | ---- | ------ | ---- |
| `10.3d` | 10.3c residue *(re-scoped 2026-08-10 after an audit read the code: BOTH halves were misst… | ⚪ pending |  |
...
| `Tool-10` | ship a first LIVE app-tool provider and consumer *(filed 2026-08-10 from an audit that re… | ⚪ pending |  |

### Object read-only lock (fleet, cross-app)

| ID | Task | Status | Gate |
| -- | ---- | ------ | ---- |
| `Lock-5(f)` | nine of nineteen guard sites survive deletion with every test still green. A mutation pas… | ⚪ pending |  |
| `Lock-5(g)` | Bookmarks is entirely ungated, and it is user-reachable data loss. mutateBookmark (apps/b… | ⚪ pending |  |
