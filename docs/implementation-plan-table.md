...
### In-place actions *(AS-5..AS-11

| ID | Task | Status | Gate |
| -- | ---- | ------ | ---- |
| `AS-5` | fragment targets: ActionFragment (kind × contentKind × host-owned anchor × lazily-materia… | ⚪ pending | OQ-AS-6 |
| `AS-6` | the request/fulfill transaction: intents.request({verb, kind, appId, fragment}) mints a s… | ⚪ pending | OQ-AS-7 |
| `AS-7` | the proposal seam: SDK useActionProposal + shared <ProposalPreview> diff/approve tray (th… | ⚪ pending | AS-6 |
| `AS-8` | wire the in-document surfaces (closes the AS-2 residue): editor inline toolbar (text sele… | ⚪ pending | AS-5, AS-7 |
| `AS-9` | first live in-place contributor: the Agent app's process:rewrite / process:translate / pr… | ⚪ pending | AS-8 |
| `AS-10` | pending / cancel / refusal UX + traces: in-flight affordance on the fragment, cancel, and… | ⚪ pending | AS-6 |
| `AS-11` | anti-rot for hot surfaces: the inline toolbar resolves on *every* text selection, so it t… | ⚪ pending | AS-8 |
