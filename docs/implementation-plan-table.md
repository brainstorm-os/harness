...
**Last updated:** 2026-07-24 — **`Agent-11d` database rows shipped** (shell `feat/agent-11d-db-rows`): the agent proposes a row in one of your databases, schema-derived from the live vault (columns + inferred types), refusing an unknown/ambiguous database and writing only that database's own columns on your approval; the tray renders it as the row it will become. Shared pieces promoted to the SDK (`decideRowCreate`/`GENERIC_OBJECT_TYPE`/`coerceScalarValue` → sdk-types, `humanizeKey` → sdk). Earlier today: **Agent-Teams track filed** (Agent-Teams-1..5, the 0.12.0 flagship, grounded in [platform/69](platform/69-agent-teams-and-orchestration.md)) and 0.9.0 shipped + four features (Agent-11c · Browser-6 · Form 8.10.5 · i18n 12.15/15d). Forward work is **release trains** to GA (`1.0.0`), then v2. 🎉 Public beta shipped 2026-06-29 (`v0.1.5`).
