...
**Read this before picking a row:** several rows are **gate-blocked by design** and should not be started ahead of their gate — `8.10.3` needs the 8.4 chrome registry, which now exists (both `8.3` and `8.4` landed 2026-07-25); `LAN-4`/`LAN-9` wait on a mandatory security review of the shell's first inbound socket; `9.18.8` waits on editor text-anchoring; `IE-10` waits on a real Anytype export fixture. Rows marked **owner** are standing tracks that ride alongside the queue rather than blocking it.
...
| 3 | `8.10.2` | Form-designer editing canvas — fill renders through the 8.3 pipeline; grid mode + per-cell subscriptions | 0.10.0 hero | `8.3` ✅ | ✅ (2026-07-25, shell #292) |
| 4 | `8.10.3` | Form-designer group nesting + chrome-cell palette | 0.10.0 hero | `8.4` ✅ (OQ-90 resolved) | ⚪ |
...
**Resume pointer:** `Browser-8` is **complete** (read-only browsing + summarize), so the top of the queue is **`Browser-5`** (row 1) — clip-to-vault, which binds the same Net-3 feeder the summarize path just proved. The train's other hero is **unblocked**: `8.3` (render pipeline) + `8.4` (chrome cells, OQ-90 resolved) landed 2026-07-25, and `8.10.2` ✅ followed. **`8.10.3` (row 4) is the last rung of Hero 1** — group nesting + the chrome-cell palette, now that a chrome cell has something to render with. Two long-lead items deserve starting out of order: **`Mailbox-9`** (row 19 — external OAuth registration, months of lead time, 🔴 for GA) and **`NAPI-P`** (row 13 — it gates every native target). The LAN ladder (rows 20–24) can progress to `LAN-3`, but **must not open a real socket** before the security review. Regenerate the source counts with `bun tools/gen-open-iterations.ts`; this linear file is hand-ordered and updated when dependency state changes.
