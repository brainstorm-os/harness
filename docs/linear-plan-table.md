...
**Read this before picking a row:** several rows are **gate-blocked by design** and should not be started ahead of their gate — `LAN-4`/`LAN-9` wait on a mandatory security review of the shell's first inbound socket; `9.18.8` waits on editor text-anchoring; `IE-10` waits on a real Anytype export fixture. Rows marked **owner** are standing tracks that ride alongside the queue rather than blocking it.
...
| 4 | `8.10.3` | Form-designer group nesting + chrome-cell palette | 0.10.0 hero | `8.4` ✅ (OQ-90 resolved) | ✅ (2026-07-25, shell #293) |
