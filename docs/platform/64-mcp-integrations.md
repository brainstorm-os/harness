...
- **OQ-MCP-1** — Server config scope: per-vault (syncs across the user's devices, like other settings) vs. per-device (a server's command line / local path may not exist on every device). Lean: per-vault config record + per-device *enablement* (the server exists in config everywhere; whether it's reachable/enabled is per-device). **Blocking** MCP-1. *[RESOLVED 2026-06-29 — per-vault config record + per-device enablement, as leaned; MCP-1..4 shipped.]*
...
- **OQ-MCP-3** — Grant granularity: is a conversation grant `mcp.server:<id>` (all of a server's tools) or down to `mcp.tool:<id>/<tool>`? Lean: server-level grant in v1 with a destructive-tool confirm gate; tool-level scopes post-v1. **Blocking** MCP-1's consent UX. *[RESOLVED 2026-06-29 — server-level grant with destructive-tool confirm gate, as leaned; tool-level scopes post-v1.]*
