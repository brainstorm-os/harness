...
- **OQ-ANS-4** *[RESOLVED in implementation-plan Agent-11b, 2026-07-23]* — when the v2 model-callable write lands: does it require a per-call confirm even when granted? **Resolved YES, and stronger than a confirm dialog: the model never *calls* a write at all.** Agent-11's create tools are **propose-not-persist** — the model stages a bounded draft (`propose-*`), and the actual `entities.create` runs only on a human approve gesture in the preview-confirm tray. So `entities.write:<type>` is exercised exclusively by a user gesture; no model output (prompt injection included) can persist. This is the general shape for every model-callable write (the `insert`-into-existing-note v2 rides the same propose→approve pattern when it lands).
