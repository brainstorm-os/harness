...
#### OQ-REPO-2 — What exactly moves, what stays, and how do the two repos stay in sync? *[RESOLVED 2026-06-25; SUPERSEDED 2026-08-25 — `harness` is now public too, with a private repo holding only the withheld material]*
...
- **Supersession (2026-08-25):** `harness` went **public** as well, so the split line moved: the design record, the dogfood harness and `tools/` are published, and a separate private repo holds only what is withheld — fundraising, commercial strategy (monetisation / pricing / payments), internal launch runbooks, credential-procurement notes, the audit trail, and the gate tooling. Publication was a filtered copy of the history (`git-filter-repo`), not a visibility flip, so the withheld material was never pushed to a public server. Withheld docs that the published set cross-references keep a stub at their path so links resolve. Of the org's repos only `cloud` and the private harness remain closed.
...
#### OQ-TOOL-1 — Argument schema language: `PropertyDef` or JSON Schema? *[RESOLVED in implementation-plan Stage 14 — Tool-3]*
...
- **Resolution (2026-08-02, owner's call — "safer wins"):** **`PropertyDef`.** Declared as `AppToolInput[]`; `validateAppToolArgs` runs at the broker before the call reaches the provider's sandbox, and `appToolInputsJsonSchema` generates the model's view from the same declaration so the enforced shape and the described shape cannot drift. Three things the leaning did not say, settled while building it:
  - **It adopts PropertyDef's value-type system and validator, not its record shape.** `key` / `icon` / `display` / `unique` / `formula` describe a stored column; an argument is a value in flight. `validateAppToolArgs` synthesizes a real `PropertyDef` and calls the shared `validateValue`, so the validator genuinely is the shared one.
  - **`richText` is refused as an argument type.** Its `validateValue` arm is a deliberate no-op (a `Y.XmlFragment` is opaque at that layer), so allowing it would have meant exactly one value type crossing the broker unchecked — the opposite of why this option was chosen.
  - **Tool arguments are validated STRICTLY; stored property values are not.** `validateValue` is a shape check and does not enforce `pattern` / `range` / `choices` / `format`. Those are layered on in `app-tool-args.ts` rather than pushed into the shared validator, because tightening the shared one would retroactively reject stored values across every vault. The asymmetry is deliberate and load-bearing: without it, a tool could declare a bound the broker never enforced.
  - Accepted cost, unchanged: no nested/structured arguments in v1. A list argument is a plain array of scalars (not the `LabeledValue` envelope — an argument has no per-value display labels). If a real case for nesting appears it is a `ValueType` extension with its own rung, which is a better problem than un-validated arguments everywhere.
- **Blocking?:** ~~Yes — blocks `Tool-3`~~ Resolved. It determines the manifest wire format, which is a compatibility surface once third-party apps declare tools.
...
#### OQ-AO-1 — Trace retention window and caps *[RESOLVED in implementation-plan Agent-12a]*
...
- **Resolution (2026-08-01): tiered, as the leaning.** `agent_events` live ~30 days; `agent_runs` ~12 months; **both additionally count-capped** (newest-wins), because an age window alone does not bound a runaway automation loop — the two limits fail differently and the schema needs both. Pruning reuses the `AiUsageRepo.prune` shape and cadence rather than inventing a second sweeper. Creation provenance on entities (`agentProvenance`) is **permanent regardless** — it lives on the entity, not in the trace, so the audit question "what made this object" survives any retention choice. The prune contract ships **inside `Agent-12a`**, not as a follow-up: an unbounded table is the failure this decision exists to prevent.
- **Blocking?:** ~~Blocks Agent-12a~~ — resolved; the rung is unblocked.

#### OQ-AO-2 — App-facing read surface *[RESOLVED in implementation-plan Agent-12a]*
...
- **Resolution (2026-08-01): shell-surfaces-only for v1 — no `agent.trace:read` capability exists.** An app sees only its own runs; the vault-wide view is the privileged shell renderer (Settings → AI). The trade is deliberately asymmetric: the capability would hand any installed app a map of which entities the user touches and when — the highest-sensitivity metadata in the vault — to serve a consumer that does not exist. Not shipping a capability is reversible; shipping one is not. Revisit only when a real consumer appears.
- **Blocking?:** No — Agent-12d ships the shell-only form.
...
- **Resolution (2026-08-01): not in v1 — metadata-only is absolute.** No path on any surface captures prompt or completion bytes. The substrate is deliberately complete without it, and the metadata timeline plus the existing message transcript answers most "why did it do that" questions. The cost of the alternative is a plaintext-prompt store that would have to be excluded from sync *and* export *and* reliably expired — three new ways to leak the most sensitive bytes in the product, to serve support cases we can mostly answer already. Revisit only with a concrete diagnostic need the timeline provably cannot meet.
- **Blocking?:** No.
...
- **Resolution (2026-08-01): active-but-coalesced in chat, passive badge for automations.** In chat the **existing Agent-5 escalation prompt is the active surface** — extend it rather than inventing a second notice mechanism (a denial the user can act on should offer the grant, not just report the refusal). Automations get a passive denial badge with denial state on the run row, because an automation denying in a loop is exactly the toast-storm case and its user is not present at the moment of refusal anyway. Shipped by `Agent-12b` / `Agent-12c`.
- **Blocking?:** No.

#### OQ-AO-5 — Model-call event granularity *[RESOLVED in implementation-plan Agent-12a]*
- **Where:** [77 §The data model](../platform/77-agent-observability.md).
...
- **Resolution (2026-08-01): no `model-call` rows at all.** The `kind` enum omits it; instead `ai_usage` gains a **nullable `run_id`** (a cheap additive migration) and the timeline derives its model steps from the join. One accounting substrate, zero duplication — and no second place for token/cost numbers to disagree with the first. Resolved inside `Agent-12a`'s schema, as the leaning proposed.
- **Blocking?:** No.
...
#### OQ-P2P-5: The discovery tag: derivation, rotation cadence, and what replaces it for multi-user *[RESOLVED in implementation-plan `P2P-1`]*
...
- **Resolution (2026-07-31, shell #377):** built, with the sub-questions answered and one deliberate strengthening. Derivation is `HKDF-SHA256(sovereign identity **SECRET**, info "brainstorm/lan-discovery/v1")` — the secret, **not** the public key the spike's sketch named, because the identity public key appears in share invites and in every wire routing header, so a tag keyed on it would be forgeable by anyone the user has ever shared with. It is reached through a session verb (`VaultSession.lanDiscoverySecret()`), the same shape as `openLanSealedChallenge`, so the sovereign secret never leaves the session. Truncation **128 bits**, epoch **5 minutes**, adjacent epoch **accepted** in both directions (±1) for clock skew between two of the user's own machines. **The tag covers the advertised port AND the sorted address list, not just the epoch** — this goes beyond the sub-question and beyond the spike's recommendation, at the cost of one more field in the MAC input: without it a sniffer can copy a live tag and re-advertise under its own address for the rest of the epoch, and the dial happens anyway; with it a replayed tag only verifies for the addresses the genuine host published, so a replay can redirect the dial to nobody. Instance names are random per boot for the same non-linking reason. The tag sits **in front of** admission and never substitutes for it. The multi-user half is unchanged and still open: two identities share no such secret, so `P2P-4` needs its own answer.
- **Blocking?:** Blocked the discovery item of `P2P-1`; resolved and shipped in the same change as the mDNS work, as required.
