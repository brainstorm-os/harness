...
- 🔴 **`ROT-3a-i` — versioned/monotonic DEK install (required before ROT-3a merges).** *(surfaced by the ROT-4 review, [_review/2026-07-09-rot-3a-security-review.md](../_review/2026-07-09-rot-3a-security-review.md).)* ROT-3a's orchestration is correct but the receive-side primitive isn't rotation-aware: `installEntityDek` no-ops on an already-installed DEK (F-ROT-1 — live survivor locked out, reproduced), `MemberWrapPayload` carries no version (F-ROT-2), and DEK ordering is receive-time so a replayed old wrap can roll a survivor back onto the revoked member's DEK (F-ROT-3). Fix: (1) bump `MEMBER_WRAP_VERSION`, add a monotonic `dekVersion` to the wrap set at mint; (2) persist it into `entity_deks.version`, rank `getByEntityId` by `version DESC`; (3) make `installEntityDek` accept strictly-newer / reject ≤ (anti-rollback); (4) extend the test to drive the real survivor `installWrap`→decrypt path. Also closes OQ-REV-6's stale-wrap-on-new-device.
