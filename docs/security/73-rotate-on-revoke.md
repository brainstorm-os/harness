...
- **`ROT-4` — verification.** A **dedicated `/security-review` + `/pentester` pass** on the whole path (this is the gate, not optional); a two-shell real-relay dogfood proving the F-286 scenario is closed (revoked shell cannot read a post-revoke edit); friction F-286 → ✅ (content).
