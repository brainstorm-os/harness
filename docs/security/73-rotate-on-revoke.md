...
- **`ROT-3a` — DEK-rotation content forward secrecy (buildable now).** Back the `RotateOnRevokePorts` (ROT-2) with real impls — `mintDek`=`EntityDekStore.persist`, `currentMembers`=`resolveCurrentMembers`, `publishWraps`=`appendWrap`+`emitWrapBootstrap`, `reSealSnapshot`=full-state emit under DEK′, `rotate`=the coordinator as a local-only no-op (§dormancy) — and hook `SharingEngine.revoke` to call `rotateOnRevoke` after the revoke append. Old-DEK grace lifetime (OQ-REV-1); collection cascade (OQ-REV-4). In-process pipeline test: revoke → rotate → a survivor decrypts a post-revoke edit; the revoked identity's old-DEK path yields nothing new.
