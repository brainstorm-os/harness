...
- 🔴 **`ROT-3a-ii` — durable deferred-rotation resume (required before merge).** *(surfaced by the ROT-4 `/security-review` re-run, [_review/2026-07-09-rot-3a-i-security-review.md](../_review/2026-07-09-rot-3a-i-security-review.md), finding F-ROT-4.)* An **offline** revoke (or an online rotation that throws mid-`publishWraps`) never rotates the DEK, `revoke()` returns `true` anyway, and the retry is structurally dead (`revokeAccess` returns `false` on a second call → the `if (revoked)` guard skips rotation) — so the removed member keeps decrypting new content. Fix: persist a durable "rotation pending for entity E" marker whenever rotation can't complete (independent of the live-grant check); drain it on relay-connect + boot; mint DEK′ locally even offline so emission flips immediately; and stop reporting `revoke()` complete when rotation was deferred.
...
- **`ROT-4` — verification.** ✅ **`/security-review` re-run done** (2026-07-09) — online path crypto/auth/injection clean across three independent lenses; surfaced F-ROT-4 → `ROT-3a-ii`. Remaining: the **`/pentester` two-shell real-relay dogfood** proving the F-286 scenario is closed (revoked shell cannot read a post-revoke edit — online AND after an offline revoke + reconnect), after `ROT-3a-ii`; friction F-286 → ✅ (content).
