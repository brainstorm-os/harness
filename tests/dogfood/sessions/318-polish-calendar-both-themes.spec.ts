/**
 * Session 316 — POLISH-APP-9 evidence: Calendar in both themes.
 *
...
test("POLISH-APP-9 — Calendar both-themes evidence", async () => {
	test.setTimeout(300_000);
	const s = await startSession("318-polish-calendar-both-themes");
	try {
		...
		s.note("POLISH-APP-9 both-themes evidence captured (calendar month grid)");
	} finally {
		await s.finish();
	}
});
