/**
 * Session 316 — POLISH-APP-10 evidence: Journal in both themes.
 *
...
test("POLISH-APP-10 — Journal both-themes evidence", async () => {
	test.setTimeout(300_000);
	const s = await startSession("319-polish-journal-both-themes");
	try {
		const jr = await s.openApp(APP.Journal);
		await jr.waitForTimeout(1500);
		...
		s.note("POLISH-APP-10 both-themes evidence captured (journal timeline)");
	} finally {
		await s.finish();
	}
});
