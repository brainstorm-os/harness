/**
 * Session 907q — HQ embeds, take two. Clean the literal "/Clients" and
 * "/Content Calendar" lines 907p left, then re-try the embed with the exact
 * human sequence that worked in 906d: click into a real paragraph, End,
 * Enter, "/", pause, query, pick from the visible menu.
 */

import { test } from "@playwright/test";
import { APP, SPEAKER, startSession } from "../lib/founder";

test("Northbound HQ: clean + embed pipeline and calendar (907q)", async () => {
	test.setTimeout(420_000);
	const s = await startSession("907q-hq-embeds");
	try {
		...
		// Clean the literal slash lines.
		for (const junk of ["/Clients", "/Content Calendar"]) {
			const line = notes.locator(`[contenteditable="true"] >> text="${junk}"`).first();
			if ((await line.count()) > 0) {
				...
				s.note(`[notes] removed literal "${junk}" line`);
			}
		}

		// Place the caret in a real paragraph, then embed.
		...
		for (const target of ["Clients", "Content Calendar"]) {
			...
			const menuOpen = await notes
				...
				.catch(() => false);
			if (!menuOpen) {
				s.note(`[notes] slash menu did NOT open for "${target}" — leaving it; filing the arm-conditions finding`);
		...
		const embeds = await notes
			...
			.catch(() => 0);
		s.note(`[notes] embed nodes in HQ now: ${embeds}`);
